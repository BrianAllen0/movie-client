...
export const MainView = () => {
  ...
  useEffect(() => {
    axios
      .get("https://ba-movie-api.herokuapp.com/movies", {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((response) => {
        const movieFetchData = response.data.map((doc) => {
          return {
            id: doc._id,
            title: doc.Title,
  ...
  return (
    <BrowserRouter>
      <Row>
        <NavigationBar setUser={setUser} setToken={setToken} />
      </Row>
        ...
        <Routes>
          <Route
            ...
            element={
              <div>
                {registering ? (
                  <RegisterView onClickLogin={() => setRegistering(null)} />
                ) : (
                  <Navigate to="/login" />
                )}
              </div>
